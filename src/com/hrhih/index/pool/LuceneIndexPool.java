...
public class LuceneIndexPool {
	...
	private int numThreads; // 同时执行索引写的线程数
	
	private int maxQueueSize; //  存放索引写的线程的队列大小
	
	private String testDir; // 测试连接是否可用的测试索引目录，默认没有测试索引目录

	private ParallelWriteIndex trackingIndexWriter = null;//索引写监控对象
	 ...
	 */
	public LuceneIndexPool(String indexDir) {
	 ...
	 *            int 连接池最大的大小
	 */
	public LuceneIndexPool(String indexDir, int initialSize,int incrementalNum, int maxSize) {
		super();
		this.indexDir = indexDir;
	 ...
	 *            int 存放索引写的线程的队列大小
	 */
	public LuceneIndexPool(String indexDir, int initialSize,int incrementalNum, int maxSize,int numThreads,int maxQueueSize) {
		super();
		this.indexDir = indexDir;
		this.initialSize = initialSize;
		this.incrementalNum = incrementalNum;
	 ...
	 *            String 测试索引库的目录
	 */
	public LuceneIndexPool(String indexDir, int initialSize,
			int incrementalNum, int maxSize,int numThreads, int maxQueueSize,String testDir) {
		super();
		this.indexDir = indexDir;
		this.initialSize = initialSize;
		this.incrementalNum = incrementalNum;
...
//		indexWriterConfig.setRAMBufferSizeMB(512);
		...
		// Real time handler
		trackingIndexWriter = new ParallelWriteIndex(indexWriter,numThreads,maxQueueSize);
	}
	...
	private synchronized ReferenceManager<IndexSearcher> newReferenceManager() {
		...
		try {
			...
			ControlledRealTimeReopenThread<IndexSearcher> writeControlThread = new ControlledRealTimeReopenThread<IndexSearcher>(
					...
					60, // when there is nobody waiting ,max sec
					10 // when there is someone waiting ,min sec
			);
