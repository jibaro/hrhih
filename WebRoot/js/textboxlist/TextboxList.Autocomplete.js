...
$.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, suggestInput, hidetimer, doAdd, currentSearch, currentRequest;
    ...
    remote: {
		...
		method: 'standard',
		placeholder: '请输入地区并选择（最多选择'+textboxlist.getOptions().max+'个）...'
	}, _options);
...
//		if ($.browser.msie) textboxlist.setOptions({bitsOptions: {editable: {addOnBlur: false}}});
		if ( /msie/.test(navigator.userAgent.toLowerCase())) textboxlist.setOptions({bitsOptions: {editable: {addOnBlur: false}}});
	...
	var search = function(bit){
		...
		if (options.queryRemote){
			...
			data["regionvalue"] = $("#"+options.suggestid).val();
...
//						alert("r=2=="+r);
	...
	var showResults = function(search){
		var boxlength=textboxlist.getBoxlength();
		if(boxlength>(textboxlist.getOptions().max-1)){
//			alert(boxlength);
			return;
		}
		
		var results = method.filter(values, search, options.insensitive, options.maxResults,options.isMatch);
	...
	var hide = function(){
		hidetimer = setTimeout(function(){
			...
			list.hide();
			currentSearch = null;			
		}, /msie/.test(navigator.userAgent.toLowerCase()) ? 150 : 0);
	};
...
$.TextboxList.Autocomplete.Methods = {
	...
	standard: {
		filter: function(values, search, insensitive, max,isMatch){
