...
$.TextboxList = function(element, _options){
	...
	var original, container, list, current, focused = false, index = [], blurtimer, events = {},boxlength;
	...
	this.getBoxlength = function(){
		return list.children('.' + options.prefix + '-bit-box').length;
	};
	...
	this.removeAllBit = removeAllBit;
	this.plugins = [];
	init();
};
	...
	var init = function(){
		...
		if (type == 'box'){
			...
			if (options.deleteButton){
				...
				close = $('<a href="javascript:void();" class="'+ typeprefix +'-deletebutton" />').click(remove).appendTo(bit);
			}
